...
# All rights reserved.
# 
# Modified by MakinoSeiran (https://github.com/MakinoSeiran)
# - Python 3.13 compatibility fixes
# - Removed deprecated string module usage
# - Updated string methods to modern Python 3 syntax
# - Fixed join/split operations for Python 3.13+
import json
import random
...
def read_key_file(filelike):
    ...
    for line in filelike:
        ...
        # split the line into words and parse the hex - 修复点
        for entry in map(lambda x: int(x, 16), line.split()):
...
def gen_ksv():
    ...
    # convert the binary to int - 修复点
    return int(''.join(ksv), 2)
...
def output_human_readable(ksv, key, is_sink):
    ...
    for idx in range(0, 40, 5):
        print(' '.join(key_strs[idx:idx+5]))  # 修复点
